...
> {
  ...
  public startDrag(activeId: string, selectedIds: string[], mouse: Point) {
  ...
  public move(mouse: Point, canvasWidth: number, canvasHeight: number): T[] {
    ...
    const targetItems = this.items.map((item) => {
      ...
      const basePosition = calculateDragPosition(
        item,
        { x: baseX, y: baseY },
        0,
        0,
