...
describe("DragEngine", () => {
  ...
  const createItems = (): TestItem[] => [
    { id: "item1", x: 100, y: 100, width: 50, height: 50, rotation: 0 },
    { id: "item2", x: 200, y: 200, width: 50, height: 50, rotation: 0 },
  ];
  ...
  describe("startDrag", () => {
    ...
    it("should record start positions for all selected items", () => {
      ...
      const result = engine.move({ x: 200, y: 200 }, canvasWidth, canvasHeight);
    ...
    it("should record pointer offset for cursor-relative dragging", () => {
      const engine = new DragEngine(createItems());
      // Mouse at (120, 120), item1 at (100, 100) â†’ offset = (20, 20)
      engine.startDrag("item1", ["item1"], { x: 120, y: 120 });
      // Move mouse to (200, 200) â†’ base position = (200-20, 200-20) = (180, 180)
      const result = engine.move({ x: 200, y: 200 }, canvasWidth, canvasHeight);
      const item1 = result.find((i) => i.id === "item1");
      // 180 snapped to grid 20 â†’ 180
  ...
  describe("move", () => {
    ...
    it("should update item positions based on mouse movement", () => {
      const engine = new DragEngine(createItems());
      engine.startDrag("item1", ["item1"], { x: 120, y: 120 });
      const result = engine.move({ x: 300, y: 300 }, canvasWidth, canvasHeight);
      const item1 = result.find((i) => i.id === "item1");
      // base = (300-20, 300-20) = (280, 280), snapped to grid 20 â†’ 280
    ...
    it("should apply grid snapping during move", () => {
      const engine = new DragEngine(createItems());
      engine.startDrag("item1", ["item1"], { x: 120, y: 120 });
      // Move to (215, 215) â†’ base = (195, 195) â†’ snapped to 200
      const result = engine.move({ x: 215, y: 215 }, canvasWidth, canvasHeight);
      const item1 = result.find((i) => i.id === "item1");
    ...
    it("should clamp position within canvas bounds", () => {
      ...
      const result = engine.move({ x: 5000, y: 5000 }, canvasWidth, canvasHeight);
      const item1 = result.find((i) => i.id === "item1");
      expect(item1!.x).toBeLessThanOrEqual(canvasWidth - item1!.width);
      expect(item1!.y).toBeLessThanOrEqual(canvasHeight - item1!.height);
    });
    ...
    it("should work with collision engine", () => {
      ...
      const result = engine.move({ x: 220, y: 220 }, canvasWidth, canvasHeight);
      const item1 = result.find((i) => i.id === "item1");
      // Collision engine should resolve non-overlapping position
      const item2 = result.find((i) => i.id === "item2");
      // item1 should not overlap item2
      const item1Rect = { x: item1!.x, y: item1!.y, width: item1!.width, height: item1!.height };
      const item2Rect = { x: item2!.x, y: item2!.y, width: item2!.width, height: item2!.height };
      // They should not overlap (with some tolerance)
      const overlaps =
        item1Rect.x < item2Rect.x + item2Rect.width &&
        item1Rect.x + item1Rect.width > item2Rect.x &&
        item1Rect.y < item2Rect.y + item2Rect.height &&
        item1Rect.y + item1Rect.height > item2Rect.y;
  ...
  describe("endDrag", () => {
    ...
    it("should clear start positions and offsets", () => {
      ...
      const result = engine.move({ x: 500, y: 500 }, canvasWidth, canvasHeight);
      expect(result).toEqual(createItems());
    });
  });
  ...
  describe("updateItems", () => {
    it("should sync internal items with external state", () => {
      const engine = new DragEngine(createItems());
      const newItems: TestItem[] = [{ id: "item3", x: 300, y: 300, width: 50, height: 50, rotation: 0 }];
      engine.updateItems(newItems);
      // Start drag on item3
      engine.startDrag("item3", ["item3"], { x: 320, y: 320 });
      const result = engine.move({ x: 400, y: 400 }, canvasWidth, canvasHeight);
